...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        Log.i(TAG, "save Token success")
    }

    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        Log.d(TAG, "From: "+ remoteMessage!!.from)
    //Notification 메세지를 수신한다면
    //remoteMessage.notification?body!! 에 내용이 저장되어있다

    //받은 remoteMassage값 출력해보기
    ...
    private fun sendNotification(remoteMessage: RemoteMessage){
        ...
        val channelName = "channelName -- 앱마다 설정"
        val channelDescription = "channelDescription -- 앱마다 설정"
        val soundUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION) //알림 소리
        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        //오레오 버전 이후에는 채널이 필요하다고함
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            val importance = NotificationManager.IMPORTANCE_HIGH //중요도. HIGH는 상단바 표시 가능
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP) // Activity Stack을 경로만 남김 (A-B-C-D-C -> A-B)

        //23.05.22 안드로이드 최신버전 대응 (FLAG_MUTABLE, FLAG_IMMUTABLE)
        //PendingIntent.FLAG_MUTABLE은 PendingIntent의 내용을 변경할 수 있도록 허용, PendgingIntent.FLAG_IMMUTABLE은 PendingIntent의 내용을 변경할 수 x

        val pendingIntent = PendingIntent.getActivity(this, uniId, intent, PendingIntent.FLAG_ONE_SHOT or
        ...
        val notificationBuilder = NotificationCompat.Builder(this, channelId)
                .setPriority(NotificationCompat.PRIORITY_HIGH) //중요도. 상단바 표시 가능
            .setSmallIcon(R.mipmap.ic_launcher) //아이콘 설정
            .setContentTitle(remoteMessage.data["title"].toString()) //제목
            .setContentText(remoteMessage.data["body"].toString()) //내용
            .setAutoCancel(true) //알람 클릭시 삭제 여부
            .setSound(soundUri) //알림 소리
            .setContentIntent(pendingIntent) //알림 실행시 intent
    ...
    fun getFirebaseToken(){
        ...
        FirebaseMessaging.getInstance().token.addOnSuccessListener {
            //람다문법
            //람다의 파라미터가 하나이고 컴파일러가 타입을 추론할 수 있는 경우 it을 사용할 수 o
            Log.d(TAG, "token=${it}")
        }
